...
- Figma URL を渡す
- figma-to-erb / figma-to-vue スキル自動選択
- figma-mcp-usage.md で Figma MCP から情報取得

<div class="grid grid-cols-2 gap-4">
<div>

- figma-to-erb
  - bem-naming.md で BEM クラス名を決定
  - erb-patterns.md で ERB テンプレート実装
  - design-tokens.md で CSS 実装

</div>
<div>

- figma-to-vue
  - vue-patterns.md で Vue テンプレート実装
  - storybook-patterns.md で Storybook 作成
  - vitest-patterns.md でテスト作成
...
Figma 情報取得の部分は共通（`figma-mcp-usage.md`）だが、<br>
...
1. **Rules: 適用範囲を明確に** — `paths` でファイルパスを限定
2. **Skills: 詳細は別ファイルへ** — 必要な時だけロード
3. **Good / Bad の両方を示す** — 禁止パターンを明示
4. **description は具体的に** — 自動呼び出し判断に影響
5. **Skills はゼロから書かなくていい** — 実作業から生成
